...
def create_user_route():
    '''Create a user and return a Users object type'''

    json_data = request.get_json()
    data: list[dict[str, str]] = []
    response: dict[str, str] = {}
    error, code = None, None
    required_fields = ['email', 'password', 'username', 'profile_img']
    if request.method == 'POST':
        if not json_data:
            ...
            if user:
                logging.info(f"User Created {user.user_id}")
    ...
    response.update(
        ...
        'error': error, 
        'code': code
        } if data and data != [] and data != [None] else {
        'sucess': False,  
        'message': 
